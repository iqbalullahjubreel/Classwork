...
public class UserManager
{
    ...
    private static string ConnectionString = "server=localhost;User=root;database=InventoryUserDataBase;password=oladapo";
    public static void CreateUserDB()
    {
        using (MySqlConnection connection = new MySqlConnection(ConnectionStringWithoutDB))
        {
            connection.Open();
            string query = "Create Database if not exists InventoryUserDataBase";
    ...
    public static void CreateUserTable()
    {
        ...
        {
            connection.Open();
            string query = "create table if not exists InventoryUserDataBase.User(id int primary Key not null auto_increment, Name varchar(255) Not Null, Age int(23), Passwords int(23), Email varchar(200) not null unique, AdminName varchar(255) Not Null, AdminPasswords int(23), AdminEmail varchar(200) not null unique);";
    ...
    public static void UpdateUser(User user)
    {
        using (MySqlConnection connection = new MySqlConnection(ConnectionString))
        {
            ...
            string Query = $"Update InventoryUserDataBase.User SET name = '{user.Name}',email ='{user.Email}',passwords ='{user.Password}' where id = {id}";
            MySqlCommand command = new MySqlCommand(Query, connection);
            var execute = command.ExecuteNonQuery();
            if (execute > 0)
            {
    ...
    public static void DeleteUser()
    {
        using (MySqlConnection connection = new MySqlConnection(ConnectionString))
        {
            ...
            int id = int.Parse(Console.ReadLine());
            string query = $"DELETE from InventoryUserDataBase.User  where id = {id};";
            MySqlCommand command = new MySqlCommand(query, connection);
            var execute = command.ExecuteNonQuery();
            if (execute > 0)
            {
    ...
    public static User LoginUser(string email, int password)
    {
        ...
        {
            connection.Open();
            string selectquery = $"SELECT id, name, email,AdminName ,AdminEmail FROM InventoryUserDataBase.User WHERE email = '{email}' AND passwords = {password} OR adminname = 'inamcojubreel@gmail.com' AND adminpasswords = 11234 ";
    ...
    public static void GetAllUser()
    {
        ...
        {
            connection.Open();
            string selectquery = "SELECT id, name, age, email FROM InventoryUserDataBase.User";
            using (MySqlCommand command = new MySqlCommand(selectquery, connection))
            {
                using (MySqlDataReader reader = command.ExecuteReader())
                {
